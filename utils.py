...
@make_nograd_func
@compute_metrics_for_each_image
def WHU_AbsDepthError_metrics(depth_est, depth_gt, mask):
    depth_est, depth_gt = depth_est[mask], depth_gt[mask]
...
@make_nograd_func
@compute_metrics_for_each_image
def Blend_Rel_Thres_metrics(depth_est, depth_gt, mask, thres):
    assert isinstance(thres, (int, float))
    depth_est, depth_gt = depth_est[mask], depth_gt[mask]
...
@make_nograd_func
@compute_metrics_for_each_image
def Blend_AbsDepthError_metrics(depth_est, depth_gt, mask):
    depth_est, depth_gt = depth_est[mask], depth_gt[mask]
    return torch.mean((depth_est - depth_gt).abs())

@make_nograd_func
@compute_metrics_for_each_image
def Blend_RelDepthError_metrics(depth_est, depth_gt, mask):
    depth_est, depth_gt = depth_est[mask], depth_gt[mask]
    return torch.mean((depth_est - depth_gt).abs() / depth_gt)

@make_nograd_func
@compute_metrics_for_each_image
def Blend_SquareRelDepthError_metrics(depth_est, depth_gt, mask):
    depth_est, depth_gt = depth_est[mask], depth_gt[mask]
...
def image_colorcode(im_, cmap="viridis", vmin=None, vmax=None, nan_color=(1, 1, 1)):
