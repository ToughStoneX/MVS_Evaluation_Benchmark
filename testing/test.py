...
from testing.gipuma import gipuma_filter
    ...
    with torch.no_grad():
        for batch_idx, sample in enumerate(TestImgLoader):
            ...
            for filename, cam, img, depth_est, photometric_confidence in zip(filenames, cams, imgs, \
                ...
                img = img.numpy()
                cam = cam[0]  #ref cam
                # print(f"depth_est: {depth_est.shape}")
                # print(f"img: {img.shape}")
                # print(f"cam: {cam}")
                depth_filename = os.path.join(args.outdir, filename.format('depth_est', '.pfm'))
                depth_color_filename = os.path.join(args.outdir, filename.format('depth_est', '.png'))
                ...
                depth_color= image_colorcode(depth_est)
                imwrite(depth_color_filename, depth_color)
